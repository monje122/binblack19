const supabaseUrl = 'https://lmectusdplbzoreybbbz.supabase.co';
const supabase = window.supabase.createClient(supabaseUrl, 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxtZWN0dXNkcGxiem9yZXliYmJ6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjA3NDU5NjYsImV4cCI6MjA3NjMyMTk2Nn0.T9xn8gl5N9kLw58hF2LUKdf7NKOpWC1GdqOMCbtcCpA');
...
async function consultarCartones() {
  ...
  data.forEach(item => {
    item.cartones.forEach(num => {
      ...
      img.src = `${supabaseUrl}/storage/v1/object/public/cartones/SERIAL_BINBLACK19_CARTON_${String(num).padStart(5, '0')}.jpg`;
...
// Crear tabla
  ...
  data.forEach(item => {
    ...
    tr.innerHTML = `
      <td>${item.nombre}</td>
      <td>
  <a href="${buildWhatsAppLink(item.telefono, `Hola ${item.nombre}, te escribo de parte del equipo de binblack19.`)}"
     target="_blank" rel="noopener">
    ${item.telefono}
  </a>
</td>
...
function abrirModalCarton(numero, elemento) {
  ...
  img.src = `${supabaseUrl}/storage/v1/object/public/cartones/SERIAL_BINBLACK19_CARTON_${String(numero).padStart(5, '0')}.jpg`;
